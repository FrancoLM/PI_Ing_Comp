...
public class PI_RabbitMQSpout extends BaseRichSpout{
    ...
    public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
        ...
        try {
            ...
            // instantiate a rabbitmq consumer, which consumes messages from the rabbitmq server queue
            Consumer consumer = new DefaultConsumer(channel) {
                @Override
                public void handleDelivery(String s, Envelope envelope, AMQP.BasicProperties basicProperties, byte[] bytes)
                        throws IOException {
                    String message = new String(bytes, "UTF-8");
                    // I may have to use the s param (consumer tag) to identify the device who sent the message...
                    try {
    ...
    public void nextTuple() {
        ...
        while ((message = messages.poll()) != null){
            ...
            // Message emitted to a bolt in the topology
